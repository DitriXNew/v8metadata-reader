...
Функция ПутьПоПолномуМетаданному(пМетаданные, компонентыПути)
	...
	ИначеЕсли имяМетаданных = "COMMONFORM" Тогда
	...
	ИначеЕсли типОбъекта = "FORM" Тогда
		...
		Если компоненты.Количество() = 5
			И ВРег(компоненты[4]) = "FORM" Тогда
			
			ДополнитьКомпоненты_Форма(компонентыПути);
			
		ИначеЕсли компоненты.Количество() > 5
			И ВРег(компоненты[4]) = "FORM"
			И ВРег(компоненты[5]) = "MODULE" Тогда
	...
	ИначеЕсли типОбъекта = "COMMAND" Тогда
		...
		Если компоненты.Количество() > 4 Тогда
			...
			Если ВРег(компоненты[4]) = "COMMANDMODULE" Тогда
...
Функция СоответствиеМодулейФайлам()
	...
	Соответствие.Вставить("OBJECTMODULE", "ObjectModule.bsl");
	Соответствие.Вставить("MODULE", "Module.bsl");
	Соответствие.Вставить("MANAGERMODULE", "ManagerModule.bsl");
	Соответствие.Вставить("RECORDSETMODULE", "RecordSetModule.bsl");
	Соответствие.Вставить("VALUEMANAGERMODULE", "ValueManagerModule.bsl");
	Соответствие.Вставить("COMMANDMODULE", "CommandModule.bsl");
	Соответствие.Вставить("PACKAGE", "Package.bin");
	Соответствие.Вставить("ORDINARYAPPLICATIONMODULE", "OrdinaryApplicationModule.bsl");
	Соответствие.Вставить("MANAGEDAPPLICATIONMODULE", "ManagedApplicationModule.bsl");
	Соответствие.Вставить("SESSIONMODULE", "SessionModule.bsl");
	Соответствие.Вставить("EXTERNALCONNECTIONMODULE", "ExternalConnectionModule.bsl");
